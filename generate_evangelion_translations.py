#!/usr/bin/env python3
"""
Generate translations and breakdowns for Evangelion episodes using Claude API.
Usage: python generate_evangelion_translations.py 01
"""
...
SYSTEM_PROMPT = """You are a Japanese language tutor helping an intermediate learner. For the given Japanese dialogue from the anime Neon Genesis Evangelion, provide:
...
2. Comprehensive vocabulary - include ALL words that an intermediate learner might not know, including:
   ...
   - Technical/military terms used in the show
...
def generate_breakdown(client, text):
    ...
    try:
        response = client.messages.create(
            ...
            messages=[{
                "role": "user",
                "content": f"Analyze this Japanese dialogue from the anime Neon Genesis Evangelion:\n\n{text}"
            }],
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python generate_evangelion_translations.py <episode_number>")
        print("Example: python generate_evangelion_translations.py 01")
        sys.exit(1)

    episode = sys.argv[1]
    episode_dir = f"evangelion/episode_{episode}"
    input_file = f"{episode_dir}/segments.json"
    output_file = f"{episode_dir}/data.json"
