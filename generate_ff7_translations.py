#!/usr/bin/env python3
"""
Generate translations and breakdowns for Final Fantasy VII Remake chapters using Claude API.
Usage: python generate_ff7_translations.py ch03
"""
...
def generate_breakdown(client, text):
    ...
    try:
        response = client.messages.create(
            ...
            messages=[{
                "role": "user",
                "content": f"Analyze this Japanese dialogue from the video game Final Fantasy VII Remake:\n\n{text}"
            }],
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python generate_ff7_translations.py <chapter_name>")
        print("Example: python generate_ff7_translations.py ch03")
        sys.exit(1)

    chapter = sys.argv[1]
    chapter_dir = f"ff7/{chapter}"
