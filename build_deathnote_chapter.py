#!/usr/bin/env python3
"""
Build listening practice data for a Final Fantasy Tactics chapter.
Usage: python build_fft_chapter.py part_01
"""
        ...
        if timestamp_match:
            ...
            while i < len(lines):
                ...
                if re.match(r'^\d+:\d+(?::\d+)?-\d+:\d+(?::\d+)?$', text_line) or text_line == '':
                    break
                # Skip "Speaker N:" prefix lines that may appear before the next timestamp
                if re.match(r'^Speaker\s+\d+\s*:\s*$', text_line):
                    break
            ...
            if text and has_japanese(text) and not should_drop(text):
                segments.append({
                    'start_sec': start_sec,
                    'end_sec': end_sec,
                    'text': text
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python build_fft_chapter.py <chapter_name>")
        print("Example: python build_fft_chapter.py part_01")
        sys.exit(1)

    CHAPTER_NAME = sys.argv[1]
    TRANSCRIPT_PATH = f"source_media/deathnote/{CHAPTER_NAME}.txt"
    AUDIO_PATH = f"source_media/deathnote/{CHAPTER_NAME}.m4a"
    CLIPS_DIR = f"deathnote/{CHAPTER_NAME}/clips"
    OUTPUT_JSON = f"deathnote/{CHAPTER_NAME}/segments.json"

    os.makedirs(f"deathnote/{CHAPTER_NAME}", exist_ok=True)
