#!/usr/bin/env python3
"""
Generate translations and breakdowns for Final Fantasy Tactics chapters using Claude API.
Usage: python generate_fft_translations.py part_01
"""
...
def generate_breakdown(client, text):
    try:
        response = client.messages.create(
            ...
            messages=[{
                "role": "user",
                "content": f"Analyze this Japanese dialogue from the video game Final Fantasy Tactics:\n\n{text}"
            }],
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python generate_fft_translations.py <chapter_name>")
        print("Example: python generate_fft_translations.py part_01")
        sys.exit(1)

    chapter = sys.argv[1]
    chapter_dir = f"fft/{chapter}"
