#!/usr/bin/env python3
"""
Build listening practice data for a Nier Replicant chapter.
Usage: python build_nier_chapter.py 01_黒ノ病
"""
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python build_nier_chapter.py <chapter_name>")
        print("Example: python build_nier_chapter.py 01_黒ノ病")
        sys.exit(1)

    CHAPTER_NAME = sys.argv[1]
    TRANSCRIPT_PATH = f"source_media/nier/{CHAPTER_NAME}.txt"
    AUDIO_PATH = f"source_media/nier/{CHAPTER_NAME}.mp3"
    CLIPS_DIR = f"nier/{CHAPTER_NAME}/clips"
    OUTPUT_JSON = f"nier/{CHAPTER_NAME}/segments.json"

    # Create output directory
    os.makedirs(f"nier/{CHAPTER_NAME}", exist_ok=True)
