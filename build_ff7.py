#!/usr/bin/env python3
"""
Build listening practice data for Final Fantasy VII Remake.
Usage: python build_ff7.py ch03
"""
...
# Audio endpoint detection settings (conservative to avoid cutting off speech)
MIN_EXTENSION = 0.8       # Always extend at least this far past transcript end
SEARCH_WINDOW = 2.5       # How far to search for silence
SILENCE_THRESHOLD = 0.08  # Lower = more conservative (less likely to cut speech)
MIN_SILENCE_DURATION = 0.25  # Require longer silence to be sure it's actually silence
FALLBACK_BUFFER = 1.2     # When no silence found, extend by this much
END_BUFFER = 0.35         # Buffer after detected silence at end
START_BUFFER = 0.15       # Buffer before detected speech at start
...
def main():
    global CHAPTER_NAME, TRANSCRIPT_PATH, AUDIO_PATH, CLIPS_DIR, OUTPUT_JSON

    if len(sys.argv) < 2:
        print("Usage: python build_ff7.py <chapter_name>")
        print("Example: python build_ff7.py ch03")
        print(f"Available chapters: {', '.join(CHAPTER_FILES.keys())}")
        sys.exit(1)

    CHAPTER_NAME = sys.argv[1]

    if CHAPTER_NAME not in CHAPTER_FILES:
        print(f"Unknown chapter: {CHAPTER_NAME}")
        print(f"Available chapters: {', '.join(CHAPTER_FILES.keys())}")
        sys.exit(1)

    source_file = CHAPTER_FILES[CHAPTER_NAME]
    TRANSCRIPT_PATH = f"source_media/{source_file}.txt"
    AUDIO_PATH = f"source_media/{source_file}.mp3"
    CLIPS_DIR = f"ff7/{CHAPTER_NAME}/clips"
    OUTPUT_JSON = f"ff7/{CHAPTER_NAME}/segments.json"

    # Create output directory
    os.makedirs(f"ff7/{CHAPTER_NAME}", exist_ok=True)

    print("=" * 80)
    print(f"BUILDING: FF7 {CHAPTER_NAME}")
    print("=" * 80)
